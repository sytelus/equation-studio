...
First-time graph compilation can be noticeable. Changing a numeric value reuses the linked program. A float field probe requires readback and should be used as an inspection action, not continuously every animation frame. The default UI's time metric is **submission time**, not a GPU timer measurement. Frame-rate counters describe the current browser session, not a promised hardware capability.
