...
Validation date: **23 September 2026**. This is an executed implementation, not a mock-up or a shader listing that was never compiled. Every figure below is generated by `tools/write_validation_report.py` from the captured test outputs.

## Executed test groups

| Group | Result | Evidence |
|---|---|---|
| JavaScript unit tests | **100 passed, 0 failed** | [Captured output](NODE_TEST_RESULTS.txt) |
...
| Editor workflows | 19 checks: UI key creation, tracked control editing and interpolation; Undo/redo preserves the animation model, selection and playhead; Dragging a key in its lane retimes it; Parameter reset restores the catalog default; Cycle rejection restores model AND visible connection menu; Raw floating-point field probe through editor; Rulers readout with raw values, pin and unpin; Wheel zoom keeps the world point under the cursor fixed; Live per-node previews render for every component; Contribution view toggles and styles through the inspector; Drag-and-drop onto a socket adds and wires a compatible component; incompatible drops are rejected; Drag wiring between dots and drag-off disconnection; Snapshot save and restore; Project JSON download and import round trip; PNG encoding + embedded UTF-8 project metadata; Four-frame deterministic PNG ZIP, CRCs, timestamps and visual change; Actual MediaRecorder video download; Context loss/recovery and float extension re-enablement | [Workflow report](WORKFLOW_VALIDATION.json) |
