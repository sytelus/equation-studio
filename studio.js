...
return {SCHEMA_VERSION,MAX_NODES,clone,makeNode,validateExpression,validateProject,topologicalOrder,parseProject,uniqueId,removeNode,History};
})();
...
function compileGraph(project, target = project.output, raw = false) {
...
return {vertexSource,compileGraph};
})();
...
return {interpolateTrack,animatedParameters,insertKey,loopTime};
})();
...
const { clone, makeNode, validateProject, parseProject, uniqueId, removeNode, History } = __modules['graph.js'];
const { presets, getPreset } = __modules['presets.js'];
const { animatedParameters, insertKey, loopTime } = __modules['timeline.js'];
const { compileGraph } = __modules['compiler.js'];
