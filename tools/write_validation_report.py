#!/usr/bin/env python3
"""Assemble measured results into the reading report; run tests before this script."""
from pathlib import Path
import json
ROOT=Path(__file__).resolve().parents[1]
gpu=json.loads((ROOT/'docs/GPU_VALIDATION.json').read_text())
workflow=json.loads((ROOT/'docs/WORKFLOW_VALIDATION.json').read_text())
native=gpu['native_image_comparison'];fields=gpu['raw_field_comparison']['fields']
...
The native draw plus readback took approximately **{native['software_backend_render_plus_readback_ms']/1000:.2f} seconds** in the software test environment. This number includes driver behavior and may include compilation; concurrent tests also affect it. It is **not a physical-GPU benchmark** and should not be used to predict a user's hardware frame rate.
...
|---|---:|---:|
'''
for name,stats in fields.items():text+=f"| {name} | {stats['mean_abs_error']:.9g} | {stats['max_abs_error']:.9g} |\n"
text+='''
