...
Optional development dependency: Pandoc. The ready-made HTML pages are included
so app users do not need Pandoc, Node, or Python merely to read the documentation.
"""
...
def main() -> int:
    ...
    if not exe:
        ...
        return 1
    sources = [ROOT/'README.md',ROOT/'ATTRIBUTION.md',ROOT/'examples/README.md',*sorted((ROOT/'docs').glob('*.md'))]
    outputs = {p.with_suffix('.html') for p in sources}
    for source in sources:
        prefix = os.path.relpath(ROOT,source.parent).replace(os.sep,'/')
        def link(path): return f'{prefix}/{path}' if prefix!='.' else path
        nav = '<nav class="topnav">' + ' '.join(f'<a href="{html.escape(link(path))}">{label}</a>' for path,label in [
            ('Equation%20Studio.html','Open studio'),('README.html','Start here'),('docs/RECIPES.html','Recipes'),('docs/ARCHITECTURE.html','Architecture'),('docs/COMPONENTS.html','Components'),('docs/RESEARCH.html','Research'),('docs/VALIDATION.html','Validation')]) + '</nav>'
        ...
        with tempfile.NamedTemporaryFile('w',suffix='.html',encoding='utf-8',delete=False) as temp:
            temp.write(nav);header=Path(temp.name)
        ...
        finally: header.unlink(missing_ok=True)
        content=output.read_text(encoding='utf-8')
        def local_reading_link(match):
            url=html.unescape(match[1])
            if '://' in url or not url.endswith('.md'):return match[0]
            target=(source.parent/url).resolve().with_suffix('.html')
            if target in outputs or target.exists():return 'href="'+match[1][:-3]+'.html"'
            return match[0]
